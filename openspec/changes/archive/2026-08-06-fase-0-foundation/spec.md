...
| REQ-05 | SQLite bootstrap (no schema) | MUST |
...
### REQ-05: SQLite Bootstrap

The app SHALL create an empty SQLite DB file at the configured `paths`/`database` location (on `init_db` or startup). **No schema/migrations** (Fase 1). All DB access SHALL go through an SQLAlchemy ORM boundary in `repositories/` so PostgreSQL is a config-only dialect swap; SQLite-only code paths MUST NOT be hard-coded.

**Acceptance**
- [ ] Empty SQLite file exists at configured path with zero tables; no migration ran.
...
#### Scenario: db created empty
- GIVEN a clean configured DB path
- WHEN the init_db/boot step runs
- THEN the file exists and contains no schema.
