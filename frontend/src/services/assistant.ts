...
export type ReportScope = "frequency" | "gap" | "average" | "probability" | "experiment";
...
export async function explainAssistant(
  ...
  return apiClient<AssistantResponse>(`/assistant/explain?${searchParams.toString()}`);
}

/** Request a Spanish interpretation of the data behind the charts. */
export async function interpretAssistant(lotteryCode: string): Promise<AssistantResponse> {
  const searchParams = new URLSearchParams({ lottery_code: lotteryCode });
  return apiClient<AssistantResponse>(`/assistant/interpret?${searchParams.toString()}`);
}
...
export async function reportAssistant(
  ...
  return apiClient<AssistantResponse>(`/assistant/report?${searchParams.toString()}`);
}

/** Summarize an experiment comparison in Spanish. */
export async function summarizeAssistant(req: SummarizeRequest): Promise<AssistantResponse> {
...
export async function assist(question: string, lotteryCode: string): Promise<AssistantResponse> {
