"""Tests for generator identity helpers — GEN-008/009 (T-GEN-018a).
...
# Golden vectors computed with the F5 primitives for the canonical inputs
# (lottery_id=7, selection_id=3, count=10, selection_fingerprint="abc123",
#  version="1.0.0"). Locked here so any accidental semantic drift fails RED.
...
GOLDEN_SEED = 297872213468358109463619875798332175481
GOLDEN_SNAPSHOT_FINGERPRINT = "ddd2dbe5c6c8002067c2191e118caf1902c9c2e9b9ee2616136119bda3feb42c"
...
class TestGenerationSeed:
    ...
    def test_locked_golden_vector(self) -> None:
        seed = generation_seed(
            GOLDEN_SELECTION_FINGERPRINT, GOLDEN_LOTTERY_ID, GOLDEN_COUNT, GENERATOR_VERSION
        )
        assert seed == GOLDEN_SEED
...
class TestSnapshotFingerprint:
    ...
    def test_deterministic_same_inputs_same_fingerprint(self) -> None:
        fp_a = snapshot_fingerprint(
            GOLDEN_LOTTERY_ID, GOLDEN_SELECTION_ID, GOLDEN_COUNT, GOLDEN_SEED, GENERATOR_VERSION
        )
    ...
    def test_locked_golden_vector(self) -> None:
        ...
        assert fp == GOLDEN_SNAPSHOT_FINGERPRINT

    @pytest.mark.parametrize(
        ("lottery_id", "selection_id", "count", "seed", "version"),
        [
            (8, GOLDEN_SELECTION_ID, GOLDEN_COUNT, GOLDEN_SEED, GENERATOR_VERSION),
            (GOLDEN_LOTTERY_ID, 4, GOLDEN_COUNT, GOLDEN_SEED, GENERATOR_VERSION),
            (GOLDEN_LOTTERY_ID, GOLDEN_SELECTION_ID, 20, GOLDEN_SEED, GENERATOR_VERSION),
            (
                GOLDEN_LOTTERY_ID,
                GOLDEN_SELECTION_ID,
                GOLDEN_COUNT,
            ...
            (GOLDEN_LOTTERY_ID, GOLDEN_SELECTION_ID, GOLDEN_COUNT, GOLDEN_SEED, "1.0.1"),
        ],
    )
    ...
    def test_cross_lottery_no_fingerprint_collision(self) -> None:
        fps = {
            snapshot_fingerprint(lottery_id, 3, 10, GOLDEN_SEED, GENERATOR_VERSION)
