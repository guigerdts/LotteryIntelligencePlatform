"""DL Provider Protocol contracts — the ONLY data seam the DL engine may import (DLE-13).

The DL Engine defines, at its composition root, two read-only provider
...
dependency (design §4, DLE-13).  Adapters wrapping repositories live at the
...
class DrawRow:
    """One draw on the official ``draw_number`` axis (DLE-03).
...
class FeatureRow:
    ...
    Mirrors ``FeatureValueRow`` from ``ml/feature_reader.py`` but is defined
    independently to enforce zero cross-engine imports (DLE-13).
    """
...
class FeatureSnapshotProvider(Protocol):
    ...
    snapshot returns ``None`` — absence is never zero-guessed (DLE-13).
    """
