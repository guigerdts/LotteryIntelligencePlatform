...
- **Read-only sobre Core Domain (G10)**: `draw`, `draw_numbers`, `super_number`, `dataset`, `import_job`, `import_error` byte-idénticos antes/después de la generación; solo aparecen filas `stat_*` / `ml_*` / `dl_*`.
